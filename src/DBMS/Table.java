...
public class Table implements Serializable
{
	...
	public void insert(String []record)
	{
		...
		recordsCount++;
		adjustBitIndices(record);
		long stopTime = System.currentTimeMillis();
		this.trace.add("Inserted:"+ Arrays.toString(record)+", at page number:"+(pageCount-1)
				+", execution time (mil):"+(stopTime - startTime));
	}

	private void adjustBitIndices(String[] record) {
		for (String col:this.indexedColumns){
			BitmapIndex bitmapIndex = FileManager.loadTableIndex(this.name,col);
			int i = Arrays.asList(this.columnsNames).indexOf(col);
			if(i == -1){
				System.err.println("error at adjust");
				return;
			}
			bitmapIndex.newRecord(record[i]);
			FileManager.storeTableIndex(this.name,col,bitmapIndex);
		}
	}
	...
	public String getFullTrace() 
	{
		...
		return res+ "Pages Count: " + pageCount + ", Records Count: " + recordsCount+", Indexed Columns: "+this.indexedColumns;
	}
	...
	public void decrementPageCount(int amount) {
		this.pageCount-=amount;
	}
	public ArrayList<String> getTrace() {
		return trace;
	}
	public void addIndexed(String col){
